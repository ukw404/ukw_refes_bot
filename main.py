...
# Key  : media_group_id (str)
# Value: {"msgs": [Message, ...], "ts": float}
# Entries older than CACHE_TTL seconds are pruned on each write.
CACHE_TTL: int = 600
...
async def accumulate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Runs in handler group -1 (before any command handler).
    Silently adds every photo/video that belongs to an album to _album.
    """
    msg = update.message
    ...
    if msg.message_id not in known:
        ...
        _album[mgid]["ts"] = time.time()   # refresh TTL on each new part
    _prune()
...
def as_input_media(msg, caption: str | None, parse_mode: str | None):
    """
    Returns InputMediaPhoto or InputMediaVideo, or None.
    Telegram send_media_group only accepts photos and videos —
    animations and documents are intentionally excluded to prevent
    API errors.
    """
    if msg.photo:
...
async def mover_referencia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1 ── Must be a reply to something ────────────────────────────────────────
    ...
    # 2 ── Replied message must contain media ──────────────────────────────────
    ...
    try:
        if mgid:
            # ── ALBUM ─────────────────────────────────────────────────────────
            # Wait briefly so accumulator has received all parts of the album
            await asyncio.sleep(1.5)

            parts = list(_album.get(mgid, {}).get("msgs", []))
            # Guarantee the target itself is included even if cache missed it
            if target.message_id not in {m.message_id for m in parts}:
                parts.append(target)
            parts.sort(key=lambda m: m.message_id)

            # Caption strictly on index 0 — prevents "Only one caption" crash
            media_list = []
        ...
        # 4 ── Clean up trigger; stamp original media permanently ──────────────
        try:
...
def main():
    ...
    # Group -1: album accumulator (runs before command handlers)
    app.add_handler(
        MessageHandler(
